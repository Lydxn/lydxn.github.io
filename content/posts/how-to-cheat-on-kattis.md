...
`Hello, World!` costs 13 characters alone, so this score should not be possible
from an entropy prospective...
...
which allows one to change the filename of the program being executed. The
...
- Replace `cowsay` with `c*`, which expands to `cowsay` since that is likely
